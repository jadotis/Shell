...
void pipeHandler(char *tokens[], int length)
{
    ...
    fprintf(stdout,"We made it through the first exec\n");
    fflush(stdout);
    }
    //Multiple pipe handling.
    while (i < numpipeflag)
    {
        fprintf(stdout, "We shouldnt be in here\n");
        fflush(stdout);
    ...
    fprintf(stdout, "Good if we made it here\n");
    fflush(stdout);
    ...
    if (finalChild == 0) //if we are in the child than we should execute again.
    {
        fprintf(stdout, "We made it to the final child with pid: %d\n", getpid());
        fflush(stdout);
    ...
    { 
            int statflag2;
            if ((finalChild = waitpid(pipeChild, &statflag2, 0)) < 0)
            {
                perror("");
            }
    }
